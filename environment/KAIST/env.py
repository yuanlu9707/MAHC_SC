...
class Env:
    ...
    def _load_charge_station_pos(self):
        ...
        # 兼容任意 charge_station_num：
        # - 若 charge_station_num < len(used_name)：只取前 charge_station_num 个，避免越界
        # - 若 charge_station_num > len(used_name)：从剩余充电站中补齐
        need = int(self.env_conf['charge_station_num'])
        if need <= 0:
            return

        picked = list(used_name[:need])
        if len(picked) < need:
            # 补齐：从全量字典里挑未使用的
            for k in all_charge_station_dict.keys():
                if k in picked:
                    continue
                picked.append(k)
                if len(picked) >= need:
        ...
        if len(picked) < need:
            raise ValueError(
                f"charge_station_num={need} exceeds available charge stations ({len(all_charge_station_dict)})."
            )

        for idx, name in enumerate(picked):
            self.charge_station_pos[idx, :] = [
                all_charge_station_dict[name][0] - self.coordx_offset,
                all_charge_station_dict[name][1] - self.coordy_offset
            ]
    ...
    def step(self, action):
        ...
        uav_collect_time = self.env_conf['epoch_time_range'] - np.minimum(
            uav_move_dis / self.env_conf['uav_speed'], self.env_conf['epoch_time_range'])
        uav_collect_time = np.maximum(uav_collect_time, 0.0)
...
def cal_uav_data_rate_mbps(uav_x, uav_y, poi_x, poi_y, uav_height):
    alpha = 4.88
    beta = 0.43
    dx = uav_x - poi_x
    dy = uav_y - poi_y
    horizontal_dis = np.sqrt(dx * dx + dy * dy)
    distance = np.sqrt(horizontal_dis * horizontal_dis + uav_height * uav_height)
    theta = np.degrees(np.arctan2(uav_height, horizontal_dis))
    path_loss = 58.46 + 20.0 * np.log10(max(distance, 1e-8)) + \
        (-19.0) / (1.0 + alpha * np.exp(-beta * (theta - alpha))) ###2GHz
    ...
    channel_power_gain = 10.0 ** ((beta_0 - path_loss) / 10.0)
    noise_power = 10.0 ** ((w_noise - 30.0) / 10.0)
    snr = pu * channel_power_gain / max(noise_power, 1e-20)
    data_rate = bandwidth * np.log2(1.0 + snr)
    if data_rate < 0.0:
        data_rate = 0.0
    return data_rate, distance
...
def fast_collect_data_or_charge(uav_collected_data, poi_num, uav_num, _cur_uav_pos_energy_hit_mec_cec_charge, min_value,
    ...
    for uid in range(uav_num):
        ...
        if not fast_is_out_of_energy(_cur_uav_pos_energy_hit_mec_cec_charge, min_value, uid) and uid not in hit_list:
            ...
            # Charging logic is intentionally disabled in no-charge setting.
            # for cid in range(charge_station_pos.shape[0]):
            #     dis = fast_dis(charge_station_pos[cid], [ux, uy])
            #     if dis < charge_sensing_range:
            #         _cur_uav_pos_energy_hit_mec_cec_charge[uid, 6] += (
            #             uav_init_energy - _cur_uav_pos_energy_hit_mec_cec_charge[uid, 2]
            #         )
            ...
            ux = _cur_uav_pos_energy_hit_mec_cec_charge[uid, 0]
            uy = _cur_uav_pos_energy_hit_mec_cec_charge[uid, 1]
            collect_time = uav_collect_time[uid]
            if collect_time <= 0:
            ...
            for pid in range(poi_num):
                if uav_collect_array[pid] == 1:
                    ...
                    if dis < uav_sensing_range:
                        ...
                        data_rate, _ = cal_uav_data_rate_mbps(ux, uy, px, py, uav_height)
                        collected_data = min(
                            _cur_poi_pos_value[pid, 2 + uav_types[uid]],
                            data_rate * collect_time / 1000.0)
