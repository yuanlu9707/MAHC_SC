...
class Env:
    ...
    def _load_charge_station_pos(self):
        ...
        # 兼容任意 charge_station_num�?        # - �?charge_station_num < len(used_name)：只取前 charge_station_num 个，避免越界
        # - �?charge_station_num > len(used_name)：从剩余充电站中补齐
...
def cal_uav_data_rate_mbps(uav_x, uav_y, poi_x, poi_y, uav_height):
    ...
    path_loss = 58.46 + 20.0 * np.log10(max(distance, 1e-8)) + \
        (-19.0) / (1.0 + alpha * np.exp(-beta * (theta - alpha)))
