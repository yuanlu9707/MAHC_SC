...
# 预测特征由主进程预计算为 log_path/pred_feat_timeseries.npy（与 train.py 一致）
_PRED_FEAT_TS_FILE = 'pred_feat_timeseries.npy'
...
def _record_action_gen_time(log_path, process_id, iter_id, action_gen_time_list):
    times = np.asarray(action_gen_time_list, dtype=np.float32)
    proc_dir = os.path.join(log_path, 'process_' + str(process_id))
    os.makedirs(proc_dir, exist_ok=True)
    np.save(os.path.join(proc_dir, 'action_gen_time_iter_' + str(iter_id) + '.npy'), times)

    if times.size == 0:
        avg_s = total_s = min_s = max_s = 0.0
    else:
    ...
    line = (
        f'iter={iter_id} process={process_id} steps={times.size} '
        f'avg_s={avg_s:.6f} avg_ms={avg_s * 1000.0:.3f} '
        f'total_s={total_s:.6f} min_s={min_s:.6f} max_s={max_s:.6f}'
    )
    with open(os.path.join(proc_dir, 'action_gen_time.txt'), 'a', encoding='utf-8') as f:
        f.write(line + '\n')
    print('[action gen time]', line)


def _init_env_obs(env, ENV_CONF, st):
    """从环境 state 生成观测（共用辅助函数）。"""
    hr_obs = np.zeros(
    ...
    obs, uid_pos, p_msk, n_msk = gen_obs(hr_obs, obs, ux, uy, env.blk_hr_grid_range,
                                         ...
                                         st['uav_types'],
                                         st['cur_uav_pos_energy_hit_mec_cec_charge'][:, 0],
                                         st['cur_uav_pos_energy_hit_mec_cec_charge'][:, 1],
                                         st['cur_uav_pos_energy_hit_mec_cec_charge'][:, 2],
                                         uid_pos, ENV_CONF['field_length'], CONF['M_size'],
                                         p_msk, n_msk, CONF['mtx_size'])
    return obs, uid_pos, p_msk, n_msk


def subp(process_id, log_path, shared_rollout, shared_ifdone,
         init_poi_value_s, ENV_CONF, Env):
    # 限制每个子进程的 numpy/sklearn/OpenMP 线程数为 1，避免各进程争抢 CPU 核
    ...
    with torch.no_grad():
        ...
        local_comm = None
        # (uav_num, max_step, train_iter)：每训练 iter、每时隙、每机的组标签
        ...
        _need_pred_assets = CONF.get('actor_use_pred', False) or (
            CONF.get('critic_use_pred', False) and _method in (
                'MAPPO', 'MAPPO_CNN', 'MAPPO_COMM'))
        pred_feat_cache = None
        _pred_feat_path = (
            os.path.join(log_path, _PRED_FEAT_TS_FILE) if _need_pred_assets else None)
                ...
                if True:
                    for uavid in range(ENV_CONF['uav_num']):
                        local_ac_list[uavid].load_state_dict(
                            torch.load(log_path + '/tmp_model_' + str(uavid) + '.pth',
                                        map_location=torch.device('cpu')))
                    if is_comm:
                        comm_path = log_path + '/tmp_comm.pth'
                    ...
                    if _need_pred_assets:
                        if not os.path.isfile(_pred_feat_path):
                            raise FileNotFoundError(
                                f'需要 pred_feat 时未找到: {_pred_feat_path}')
                        pred_feat_cache = np.load(_pred_feat_path)
                        if pred_feat_cache.shape[0] != ENV_CONF['max_step']:
                            raise ValueError(
                                f'{_PRED_FEAT_TS_FILE} 第一维应为 max_step={ENV_CONF["max_step"]}, '
                                f'实际 {pred_feat_cache.shape}')
                        _d = CONF.get('pred_out_d', 64)
                        if pred_feat_cache.shape[1] != _d:
                            raise ValueError(
                                f'{_PRED_FEAT_TS_FILE} 第二维应为 pred_out_d={_d}, '
                                f'实际 {pred_feat_cache.shape}')
                    st = env.reset(init_poi_value_s)
                    obs, uid_pos, p_msk, n_msk = _init_env_obs(env, ENV_CONF, st)
                    ...
                    last_action_np = np.zeros([ENV_CONF['uav_num'], CONF['action_space']],
                                              dtype=np.float32)
                    action_gen_time_list = []
                    ...
                    for step_id in range(ENV_CONF['max_step']):
                        ...
                        cur_pred_feat = None
                        if _need_pred_assets and pred_feat_cache is not None:
                            cur_pred_feat = torch.from_numpy(
                                pred_feat_cache[step_id:step_id + 1])

                        action_gen_t0 = time.perf_counter()
                        if is_comm:
                            comm_alpha = float(get_comm_alpha(iter_i=sub_iter_counter, test=False))
                            ...
                            h_comm, _ = local_comm(
                                z_all, update_groups=True,
                                force_cluster=(
                                    CONF.get('comm_cluster_at_slot0', True)
                                    and step_id == 0))

                            # 将本时隙的分组标签写入共享 Buffer（轨迹数据）
                            _gl = torch.tensor(
                            ...
                            if critic_use_comm:
                                shared_rollout.h_comm_s[step_id, 0].copy_(
                                    (h_comm * comm_alpha).detach()[0])
                            value_list = []
                            action_list = []
                            action_log_prob_list = []
                            for uid in range(ENV_CONF['uav_num']):
                                val, act, alp = local_ac_list[uid].get_action_from_z(
                                    x_embed_list[uid], z_list[uid], h_comm[:, uid], obs_all=obs_all_flat,
                                    h_comm_all=h_comm_all, pred_feat=cur_pred_feat, comm_alpha=comm_alpha)
                            ...
                            h_list = []
                            for uid in range(ENV_CONF['uav_num']):
                                _pf_s = cur_pred_feat if _need_pred_assets else None
                                value, action, action_log_prob, h_uid = local_ac_list[uid].get_action_s(
                                    shared_rollout.obs_s[step_id, :, uid],
                                    shared_rollout.recurrent_hidden_states_s[step_id, :, uid],
                                ...
                                h_list.append(torch.unsqueeze(h_uid, 1))
                            value_s = torch.cat(value_list, 1)
                            action_s = process_action(action_list)
                            action_s_log_prob = torch.cat(action_log_prob_list, 1)
                            h = torch.cat(h_list, 1)
                        action_gen_time_list.append(time.perf_counter() - action_gen_t0)
                        ...
                        shared_rollout.insert(torch.tensor(obs, dtype=torch.float32),
                                              ...
                                              pred_feat=cur_pred_feat)
                        last_action_np = action_s.copy()

                    obs_all_last = shared_rollout.obs_s[-1]
                    obs_all_last_flat = obs_all_last.view(
                        obs_all_last.size(0), -1, *CONF['obs_shape'][1:])
                    _bootstrap_pf = None
                    if (CONF.get('critic_use_pred', False)
                            and pred_feat_cache is not None):
                        _ix = ENV_CONF['max_step'] - 1
                        _bootstrap_pf = torch.from_numpy(
                            pred_feat_cache[_ix:_ix + 1])
                    _bootstrap_h_comm = None
                    if is_comm and CONF.get('critic_use_comm', False):
                        # 复用采样时隙 T-1 的 h_comm（与最后一步 critic 输入同源），不再额外前向
                    ...
                    for uid in range(ENV_CONF['uav_num']):
                        next_value = local_ac_list[uid].get_value_s(
                            obs_all_last_flat, pred_feat=_bootstrap_pf,
                            h_comm_all=_bootstrap_h_comm)
                    ...
                    # _record_action_gen_time(log_path, process_id, sub_iter_counter, action_gen_time_list)
                    shared_rollout.compute_returns(next_value_s, use_gae=True,
                                                   gamma=CONF['gamma'], tau=CONF['tau'])
                    ...
                    if sub_iter_counter >= CONF['train_iter'] - 1000:
                        _bundle = trace_plot_bundle_from_episode_info(sublog.episode_info, ENV_CONF)
                        _eph = os.path.join(log_path, 'process_' + str(process_id), 'trace_plot_last.pkl')
                        with open(_eph, 'wb') as _f:
